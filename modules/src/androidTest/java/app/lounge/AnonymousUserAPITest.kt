...
class UserAuthenticationAPITest {
    ...
    fun test1OnSuccessReturnsAuthData() = runBlocking {
        val response = userAuthentication.requestAuthData(
    ...
    fun test2OnSuccessReturnsLoginData() = runBlocking {
        ...
        authData?.let { authData ->
            ...
            val response = userAuthentication.requestAuthDataValidation(
    ...
    fun test3OnTimeoutFailureReturnsError(): Unit = runBlocking {
        ...
        authData?.let { authData ->
            ...
            try {
                val response = userAuthenticationGoogleTimeout.requestAuthDataValidation(
                    anonymousAuthDataValidationRequestBody = AnonymousAuthDataValidationRequestBody(
                        authDataResponse = authData
                    )
                )
    ...
    private val eCloudTest = retrofitTestConfig(UserAuthenticationRetrofitAPI.tokenBaseURL)
    private val googleTest = retrofitTestConfig(UserAuthenticationRetrofitAPI.googlePlayBaseURL)
    private val googleTestTimeout = retrofitTestConfig(
        UserAuthenticationRetrofitAPI.googlePlayBaseURL, 50L)

    private val userAuthentication: UserAuthentication = UserAuthenticationRetrofitImpl(
        eCloud = eCloudTest,
        google = googleTest
    )

    private val userAuthenticationGoogleTimeout: UserAuthentication = UserAuthenticationRetrofitImpl(
        eCloud = eCloudTest,
